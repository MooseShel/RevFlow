...
/**
 * HIPAA-Compliant Sanitizing Logger
 * Automatically redacts Patient Health Information (PHI) and financial numbers.
 */

// Regex patterns for automatic string redacting
...
/**
 * Redacts typical PHI string patterns.
 */
...
/**
 * Deeply sanitizes objects or metadata values to strip out sensitive keys.
 */
export function sanitizeMetadata(metadata: any): any {
  ...
  for (const [key, value] of Object.entries(metadata)) {
    ...
    // Check key patterns representing PHI or financial amounts
    if (
      ...
      (lowerKey.includes("key") && !lowerKey.includes("token")) // keep token_id, token
    ) {
...
/**
 * Formats a log output as standard JSON for logging management solutions (e.g. Railway, Datadog)
 */
