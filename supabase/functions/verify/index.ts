...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const result = await runInRLSTransaction({ tokenId: token }, async (sqlTx) => {
      ...
      // 4. Verify match against ZIP Code
      const zipMatch = tokenRecord.hashed_zip && hashedInput === tokenRecord.hashed_zip;

      if (!zipMatch) {
        logger.warn("Verification failed: incorrect details", { token, ip: ipAddress });
        await sqlTx`
          INSERT INTO access_audit_logs (token_id, event_type, ip_address, user_agent, metadata)
          VALUES (${tokenRecord.token_id}, 'ATTEMPT_FAIL', ${ipAddress}, ${userAgent}, ${JSON.stringify({ reason: "Incorrect ZIP Code" })})
        `;
